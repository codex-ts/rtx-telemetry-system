...
# ⚙️ TOOL DEFINITION: The AI Agent will execute this function ONLY when required
def fetch_recent_hardware_logs() -> str:
    ...
    try:
        ...
        return f"CRITICAL: Failed to query database: {str(e)}"

@app.post("/api/copilot/analyze")
async def analyze_hardware_vitals(request: ChatRequest):
    try:
        ...
        # Fire the content generation loop with the tool attached
        ...
        raise HTTPException(status_code=500, detail=f"Copilot Pipeline Exception: {str(e)}")
